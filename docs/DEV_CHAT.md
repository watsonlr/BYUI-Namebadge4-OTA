...
### Display — stable hardware orientation and splash rendering

Confirmed a stable display configuration on multiple eBadge V4 boards:

- ILI9341 MADCTL (`0x36`) is `0x00`.
- Logical 320×240 landscape maps CASET to logical `y`.
- RASET is inverted (`DISPLAY_W - 1 - x`) so logical `x=0` appears at the mounted left edge.
- Built-in 8×8 font tables are rendered MSB-left so individual letters read correctly.
- Normal drawing paths use host-order RGB565 and `pack_pixel()`.
- Splash image rows are generated by `splash_screen/png_to_rgb565.py` as byte-swapped RGB565. `display_draw_row_raw()` mirrors each row left-right and sends those bytes directly; repacking the raw splash pixels corrupts BYUI blue into brown/green while white remains mostly unchanged.

Verified on hardware: text orientation, text colours, splash orientation, and splash blue/white colour rendering are correct.

**Files changed:** `display/display.c`, `display/include/display.h`, `docs/HARDWARE.md`, `CLAUDE.md`, `docs/DEV_CHAT.md`
